...
class Permutation:
    ...
    def images(self, val: list[int]):
        ...
        # Permet d'éviter les problèmes d'indice, puisqu'on compte à partir de 1
        val.insert(0, 0)

        if not Permutation._est_valide(val):
            raise ValueError(
                "La liste donnée ne définit pas une permutation valide")
    ...
    def __mul__(self, autre):
        ...
        if isinstance(autre, Permutation):
            if len(autre._images) != (self.n + 1):
                raise ValueError(
                    "Multiplication de deux permutations sur des ensembles différents.")
    ...
    def produit_transpositions(self):
        ...
        if not self._produit_transpositions[0]:
            ...
            for k in range(len(self.representation_cycle) - 1):
                self._produit_transpositions[1].append(
                    Cycle((self.representation_cycle[k], self.representation_cycle[k + 1]), self.n))
