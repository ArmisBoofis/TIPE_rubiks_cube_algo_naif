...
class VecteurOrientation():
    ...
    def __add__(self, autre):
        ...
        if isinstance(autre, VecteurOrientation):
            if len(autre.valeurs) != self.n or autre.modulo != self.modulo:
                raise ValueError(
                    "Impossible d'additionner deux vecteurs orientations de caractéristiques différentes (taille, modulo).")
                ...
                for k in range(self.n):
                    nouvelles_valeurs.append(
                        self._valeurs[k] + autre.valeurs[k])
            ...
            raise TypeError(
                f"Impossible d'additionner un vecteur orientation avec un objet de type {type(autre)}")
    ...
    def __rmul__(self, perm):
        ...
        if isinstance(perm, Permutation):
            ...
            if perm.n == self.n:
                # On initalise un nouveau vecteur orientation vierge
                arg_valeurs = [0] * self.n
                ...
                raise ValueError(
                    "Multiplication d'un vecteur orientation par une permutation de mauvaise taille.")
